...
GrammarII GrammarII::get_without_unproductive_symbols()
{
	set<string> n_i;
	set<string> n_im1;
...
GrammarII GrammarII::get_without_inaccessible_symbols()
{
...
GrammarII GrammarII::get_reduced_form()
{
	auto g = get_without_unproductive_symbols();
	return g.get_without_inaccessible_symbols();
}

GrammarII GrammarII::get_erase_eps_transitions()
{
	set<string> n_0;
	for (auto& rule : rules)
	{
	...
	function < void(unsigned, unsigned, vector<string>, set<vector<string>> &) > bkt =
		[&](unsigned idx, unsigned max_size, vector<string> vec, set<vector<string>>& set)
		// idx = 0, maxsize = vec.size()
		{
			if (idx == max_size)
			{
...
GrammarII GrammarII::get_erase_renaming_transitions()
{
...
bool GrammarII::is_in_chomsky_form()
{
	for (auto& rule : rules)
	{
		for (auto& concat_symbol : rule.second)
...
GrammarII GrammarII::get_in_chomsky_form()
{
...
bool GrammarII::test_with_cyk(std::string word)
{
	unsigned n = word.size();
	
	vector<vector<set<string>>> v_table;
	v_table.resize(word.size());

	unsigned r_size = n;
	for (auto& row : v_table)
		row.resize(n);
	...
	for (unsigned i = 0; i < n; i++)
	{
		...
		for (auto& rule : rules)
		{
			for (auto& concat_symbol : rule.second)
			{
				if (concat_symbol.size() == 1 and concat_symbol[0] in terminals and concat_symbol[0][0] == word[i])
					nts.insert(rule.first);
			}
		}
	...
	for (unsigned j = 1; j < n; j++) {
		for (unsigned i = 0; i < n - j; i++) {
			...
			auto switch_table = v_table;

			printf("For V[%d,%d]: \n", i, j);
			
			for (unsigned k = 0; k < j; k++)
			{
				printf("\tLooking at V[%d,%d] X V[%d,%d]\n", i, k, i + k + 1, j - k - 1);
				
				for (auto& s1: v_table[i][k])
				{
					for (auto& s2 : v_table[i + k + 1][j - k - 1])
					{
						for (auto& rule : rules) {
							if (vector<string>({ s1, s2 }) in rule.second)
								switch_table[i][j].insert(rule.first);
						}
					}
				}
			}

			v_table = switch_table;
		}
	}

	return start_symbol in v_table[0][n - 1];
}

string GrammarII::to_text(bool prettify)
{
	if (prettify)
	{
